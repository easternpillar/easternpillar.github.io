...
> 하드웨어 자원뿐만 아니라 쿠버네티스 리소스에 대한 제약을 정의할 수 있으며 여러 ResourceQuota 간에 충돌이 발생할 경우 더 엄격한 제약사항을 가진 기준을 따른다. 
{: .prompt-tip }
...
> 클러스터, 사용자, 네임스페이스 및 인증 메커니즘에 대한 정보를 관리하는 오브젝트

> 클러스터, 사용자, 네임스페이스를 이용하여 컨텍스트를 형성하고 인증 정보를 관리하여 API 호출시 인증과 관련된 필드 입력을 생략할 수 있다.
{: .prompt-tip }
...
> 쿠버네티스 리소스가 점유할 수 있는 자원의 범위(기본값과 제한)를 정의하는 오브젝트
