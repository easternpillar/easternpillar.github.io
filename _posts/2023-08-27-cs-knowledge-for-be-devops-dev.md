...
> 멀티 프로세싱과 멀티태스킹: 멀티 프로세싱은 여러개의 코어에서 여러개의 프로세스가 동시에 실행될 수 있는 것을 말하고, 멀티태스킹은 하나의 코어에서 여러 개의 프로세스 또는 쓰레드가 문맥교환을 통해 실행될 수
> 있다.
> {: .prompt-info }
...
> 1. 삽입 이상: 새로운 데이터 입력 시 불필요한 데이터가 함께 삽입되는 이상
> 2. 삭제 이상: 데이터 삭제 시 의도치 않은 데이터가 함께 삭제되는 이상
> 3. 갱신 이상: 데이터 갱신 시 중복된 데이터로 인해 일관성으로 저해되는 이상
     > {: .prompt-info }
